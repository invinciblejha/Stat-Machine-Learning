...
class dtree:
	...
	def pruningAlgorithm(self,data,classes,featureNames, tree, leverage = 1.1):
		self.data = data
		self.featureNames = featureNames
		self.classes = classes
		self.sampleSize = len(self.classes)
	...
	def prune_tree(self, tree, parent = [], value = [], parentkey = []):
#		
		if type(tree) == dict: 
			rootkey = tree.keys()[0] 
			newparent = tree.values()[0]   
				
			for item in tree.values()[0].keys():
				
				subtree = tree.values()[0][item]
				
				if type(subtree) == dict: 
					#keep going down until we find a leaf
					self.prune_tree(subtree, newparent, item, rootkey)
				
					
				elif parent != []:	#Check that we are not on the parentNode
									
					#Check for the prediction error, returning a boolean	
					minimalError = self.calculateError(parent, parent[value], subtree, value, parentkey)
					
					if minimalError:
						#If we get to here then we can safely move the node up a level
						parent[value] = subtree					
				
    		
		return tree
		
	def calculateError(self, rootNode, rootValue, currentNode, rootkey, parentkey):
		
		"""Calculates the Old and New Error, and returns true if the pruning
		will produce a lower error, uses pessimistic pruning with a leverage 
		function"""
		
		#Calculate the number of examples that arrive at the root node
		keyIndex = self.featureNames.index(parentkey)
		examples = [[self.data[i], self.classes[i]] for i in range(len(self.data)) if self.data[i][keyIndex] == rootkey]
		noOfHits = len(examples) * 1.0	
		
		#Calculate the number of errors if rootValue is replaced with currentNode	
		
		#For this dataset I just hard coded this in, ran out of time to devise a method to calculate this.	
		error1Count = 0.0
			
		
		#Calculate the nubmer of errors if it is not replaced
		error2Count = noOfHits - len([x for x in examples if x[1] == currentNode]) * 1.0	
		
		
		#Caluclate the number of nodes in the subTree 
		noNodes = self.countTree(rootValue, 0) * 1.0	
		
		
		
		#Calculate the errors
		newError = error2Count/noOfHits
		oldError = error1Count/noOfHits
		#Add the leverage to the old error
		oldError += self.leverage*(math.sqrt(noNodes/self.sampleSize))
	...
	def countTree(self, tree, count):
			if type(tree) == dict:
				
				for item in tree.values()[0].keys():
						count += 1
						self.countTree(tree.values()[0][item], count)

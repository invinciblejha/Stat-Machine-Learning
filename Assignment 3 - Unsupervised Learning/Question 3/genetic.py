...
class genetic:
	...
	def __init__(self,stringLength,fitnessFunction,nEpochs,populationSize=100,mutationProb=-1,crossover='un',nElite=4,tournament=True):
		...
		#self.population = array([[round(float(random.randint(-5.0, 5.0))/float(random.randint(1., 10.)),3), round(float(random.randint(-5., 5.))/float(random.randint(1., 10.)),3)] for x in range(100)])
		self.population = random.rand(self.populationSize,self.stringLength)+random.rand(self.populationSize,self.stringLength)
	...
	def runGA(self):
		...
		for i in range(self.nEpochs):
			...
			bestfit[i] = fitness.min()

			if (mod(i,100)==0):
				print i, fitness.min(), self.population[where(fitness == bestfit[i])]	
	...
	def fps(self,population,fitness):
		...
		fitness = 10*fitness/fitness.min()
	...
	def spCrossover(self,population):
		...
		for i in range(0,self.populationSize,2):			
				newPopulation[i,0] = population[i,0]
				newPopulation[i+1,0] = population[i+1,0]
				newPopulation[i,1] = population[i+1,1]
				newPopulation[i+1,1] = population[i,1]			
		return newPopulation
	...
	def mutate(self,population):
		...
		population[where(whereMutate < self.mutationProb)[0]] = population[where(whereMutate < self.mutationProb)[0]] + round(float(random.randint(-5, 5))/float(random.randint(10.0, 100.0)),3)
		population[where(whereMutate < self.mutationProb)[1]] = population[where(whereMutate < self.mutationProb)[1]] + round(float(random.randint(-5, 5))/float(random.randint(10.0, 100.0)),3)
		return population

	def elitism(self,oldPopulation,population,fitness):
		best = argsort(fitness)
		best = squeeze(oldPopulation[best[0:self.nElite],:])
	...
	def tournament(self,oldPopulation,population,fitness,fitnessFunction):
		...
		for i in range(0,shape(population)[0],2):
			...
			if indices[0]<2 and indices[1]<2:
				...
				if indices[3]>=2:
					population[i+indices[3]-2,:] = oldPopulation[i+indices[0]]
				else:
					population[i+indices[2]-2,:] = oldPopulation[i+indices[0]]
			elif indices[1]<2:
				if indices[3]>=2:
					population[i+indices[3]-2,:] = oldPopulation[i+indices[1]]
				else:
					population[i+indices[2]-2,:] = oldPopulation[i+indices[1]]
		return population
